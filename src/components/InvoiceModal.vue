...
import { uid } from "uid";
import db, {collection, addDoc, updateDoc, doc } from '../firebase/FirebaseInit';
  ...
  methods: {
    ...
    async uploadInvoice(){
      if(this.invoiceItemList.length <= 0){
        alert("Please make sure you have enter invoice details");
        return;
      }
    ...
    async updateInvoice() {
      ...
      this.loading = true;
      this.calInvoiceTotal();
      const ref = doc(db, "invoices", this.docId);
      await updateDoc(ref,{
        billerStreetAddress: this.billerStreetAddress,
        billerCity: this.billerCity,
        billerZipCode: this.billerZipCode,
